...
public class RobotAutoDriveByGyroMecanum extends LinearOpMode {
    ...
    public void runOpMode() {
        motors = new Motor[]{new Motor("front_left_drive", DcMotor.Direction.REVERSE),
                new Motor("front_right_drive", DcMotor.Direction.FORWARD),
                new Motor("back_left_drive", DcMotor.Direction.REVERSE),
                new Motor("back_right_drive", DcMotor.Direction.FORWARD)};
        imu = InitIMU();
        waitForStart();
    ...
    void rotate(double angle_deg) {
        driveDistance(0, rotation_deg-angle_deg, P_TURN_GAIN);
    ...
    void driveDistance(double distance, double heading, double proportionalGain) {
        if (opModeIsActive()) {
            ...
            double drive_x = Math.sin(heading)*distance;
            double drive_y = Math.cos(heading)*distance;
            motors[0].target = motors[0].drive.getCurrentPosition() + (int)((drive_y + drive_x) * moveCounts);
            motors[1].target = motors[1].drive.getCurrentPosition() + (int)((drive_y - drive_x) * moveCounts);
            motors[2].target = motors[2].drive.getCurrentPosition() + (int)((drive_y - drive_x) * moveCounts);
            motors[3].target = motors[3].drive.getCurrentPosition() + (int)((drive_y + drive_x) * moveCounts);
            for (Motor motor: motors) {
            ...
            while (opModeIsActive() && Arrays.stream(motors).allMatch(motor -> motor.drive.isBusy())) {
                double turnSpeed = getSteeringCorrection(rotation_deg, proportionalGain);
                if (drive_y < 0) {
                    turnSpeed *= -1.0;
                }
