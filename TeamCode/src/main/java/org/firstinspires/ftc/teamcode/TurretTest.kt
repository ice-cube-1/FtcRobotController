...
class TurretTest : LinearOpMode() {
    ...
    private fun wrap() { turret.setPower(if (goto < turret.getPosition()) -TURRET_SPEED else TURRET_SPEED) }
    private fun centerOnTag() { turret.setPower(-lastAngle * TURRET_ENCODER_KP) }
    private fun scan() {
        if (scanMin < scanMax) {
            if ((toScanMin && scanMin > turret.getPosition()) || (!toScanMin && scanMax < turret.getPosition())) {
                toScanMin = !toScanMin
            }
