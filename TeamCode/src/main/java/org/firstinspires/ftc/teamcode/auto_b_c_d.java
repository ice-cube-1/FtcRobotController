...
public class auto_b_c_d extends auto {
    ...
    public void runOpMode() {
        start_x = Math.sqrt(2*Math.pow(robot_length_inches, 2))/2; // presuming exactly 45 degree angle
        start_y = 100; // CHANGE THIS
        initial_rotation = 45; // PREFERABLY CHANGE THIS? Depends how easy it is to lay the robot out
        ...
        driveToPoint(robot_length_inches*3/2 + 2, field_length-basket_distance_from_corner);
        driveToPoint(robot_length_inches*3/2 + 2, robot_length_inches*1/2 + 2);

        // extra tolerance to not hit other robot (change dependent on other robot's parking mechanism
        // and if we park first)
        driveToPoint(robot_length_inches/2 + 2, robot_length_inches*3/2 + 2);
